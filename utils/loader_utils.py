...
def get_stamp_list(dataset, timestamp):
    ...
    print(
        f"----know the dataset dataset poses = {len(dataset.dataset.poses)} and frame length = {frame_length}, anf len dataset {len(dataset)}"
    )
    ...
    return [
        dataset[i * frame_length + timestamp] for i in range(len(dataset.dataset.poses))
    ]
