...
class GaussianModel:
    ...
    def get_base_coefficient(self, step: int, total_steps=12000):
        if step > total_steps:
            return 1
        else:
            return 0.01 + (step / total_steps) * (1 - 0.01)
    ...
    def set_appearance(self, num_cameras):
        if self.appearance_dim > 0:
            print(f"appearance dim = {self.appearance_dim}")
    ...
    def create_from_pcd(
        self, pcd: BasicPointCloud, spatial_lr_scale: float, time_line: int
    ):
        ...
        self._deformation_table = torch.gt(
            torch.ones((self.get_xyz.shape[0]), device="cuda"), 0
        )
    ...
    def compute_deformation(self, time):
        ...
        xyz = self._anchor + deform
        return xyz

    def save_deformation(self, path):
        torch.save(
            self._deformation.state_dict(), os.path.join(path, "deformation.pth")
        )
    ...
    def cat_tensors_to_optimizer(self, tensors_dict):
        ...
        for group in self.optimizer.param_groups:
            if (
                ...
                or "grid" in group["name"]
                or "deformation" in group["name"]
            ):
                continue
            # assert len(group["params"]) == 1 print(
    ...
    def get_displayment(self, selected_point, point, perturb):
        ...
        displacements = (
            torch.randn(selected_point.shape[0], 3).to(selected_point) * perturb
        )
    ...
    def add_new_gaussians(self, number):
        new_scaling = torch.ones_like(self._scaling[:number]).float().cuda()  # *0.05
        # new_scaling = torch.log(new_scaling)
        new_rotation = torch.zeros([number, 4], device="cuda:0").float()
        new_rotation[:, 0] = 1.0

        new_opacities = inverse_sigmoid(
            0.1 * torch.ones((number, 1), dtype=torch.float, device="cuda")
        )

        # new_feat = self._anchor_feat.unsqueeze(dim=1).repeat([1, self.n_offsets, 1]).view([-1, self.feat_dim])[candidate_mask]
        new_feat = self._anchor_feat[:number].cuda()
        # new_feat = scatter_max(new_feat, inverse_indices.unsqueeze(1).expand(-1, new_feat.size(1)), dim=0)[0][remove_duplicates]
        # self._anchor
        new_offsets = torch.zeros_like(self._offset[:number]).float().cuda()
        positions = (10 + 10) * torch.rand((number, 3)) + 10
        d = {
            "anchor": positions.cuda(),
            "scaling": new_scaling,
        ...
        print("cuda is", self.anchor_demon.device)
    ...
    def adjust_anchor(
        ...
        prune_mask = (
            self.opacity_accum < min_opacity * self.anchor_demon * self.n_offsets
        ).squeeze(dim=1)
    ...
    def load_mlp_checkpoints(self, path, mode="split"):  # split or unite
        ...
        weight_dict = torch.load(
            os.path.join(path, "deformation.pth"), map_location="cuda"
        )
