...
class Deformation(nn.Module):
    def __init__(
        ...
        # self.deformation_net = Deformation(W=64, D=1, input_ch=63, grid_pe=0, input_ch_time=32, args=hyper)
    ...
    def create_net(self):
        ...
        self.feat_deform = nn.Sequential(
            nn.ReLU(), nn.Linear(self.W, self.W), nn.ReLU(), nn.Linear(self.W, 32)
        )
    ...
    def forward(
        ...
        if time_emb is None:
            ...
            return self.forward_dynamic(
                rays_pts_emb, scales_emb, rotations_emb, opacity, time_feature, time_emb
            )
    ...
    def forward_dynamic(
        ...
        feat = self.feat_deform(hidden)
        # print(f"pts= {pts.shape} , scales {scales.shape}, rotations = {rotations.shape}, opacity = {opacity.shape}")
        return pts, scales, rotations, opacity, feat
...
class deform_network(nn.Module):
    ...
    def forward(
        self, point=None, scales=None, rotations=None, opacity=None, times_sel=None
    ):
    ...
    def forward_dynamic(
        self, point, scales=None, rotations=None, opacity=None, times_sel=None
    ):
        ...
        means3D, scales, rotations, opacity, feat = self.deformation_net(
            point_emb, scales_emb, rotations_emb, opacity, None, times_sel
        )
        return means3D, scales, rotations, opacity, feat
