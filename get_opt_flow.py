...
class OccMask(torch.nn.Module):
    ...
    def get_oob_mask(self, base_coord, flow_1_2):
        ...
        oob_mask = (
            (target_range[..., 0] < 0)
            | (target_range[..., 0] > self.W - 1)
            | (target_range[..., 1] < 0)
            | (target_range[..., 1] > self.H - 1)
        )
...
def get_flow(pair_imgs, sintel_ckpt=True):  # TODO: test with gt flow
    ...
    flow_net = (
        ...
        if sintel_ckpt
            ...
            "submodules/third_party/RAFT/models/Tartan-C-T-TSKH-spring540x960-M.pth"
        )
    )
    ...
    with torch.no_grad():
        ...
        for i in range(2):
            print(i)
            imgs_ij = [
                torch.tensor(pair_imgs[i][0]).float().to(device),
                torch.tensor(pair_imgs[i][1]).float().to(device),
            ]
            flow = flow_net(
                imgs_ij[0] * 255, imgs_ij[1] * 255, iters=20, test_mode=True
            )[1]
            flow_ij.append(flow)
            save_optical_flow(flow, output_folder, f"optical_flow{i}_1.png")

            flow1 = flow_net(
                imgs_ij[1] * 255, imgs_ij[0] * 255, iters=20, test_mode=True
            )[1]
        ...
        for i in range(2):
            save_optical_flow(
                flow_ij[i], output_folder, f"mask flow_i{i}.png", valid_mask_i[i]
            )
            ...
            save_optical_flow(
                flow_ji[i], output_folder, f"mask flow_j{i}.png", valid_mask_j[i]
            )
