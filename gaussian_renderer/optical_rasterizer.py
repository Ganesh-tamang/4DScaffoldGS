...
def opt_render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0,stage="fine", override_color = None,cam_type=None, retain_grad=False, step=16000, show_anchor=False, opacity_limit=0.005):
    """
    Render the scene. 
    ...
    if "coarse" in stage:
        ...
        # feat = pc._anchor_feat
    radii_pure = rasterizers.visible_filter(means3D = means3D_final,
        scales = anc_scales[:,:3],
    ...
    rendered_image, radii, rendered_depth, rendered_alpha, proj_means_2D, conic_2D, conic_2D_inv, gs_per_pixel, weight_per_gs_pixel, x_mu = rasterizers(
        means3D = xyz ,
